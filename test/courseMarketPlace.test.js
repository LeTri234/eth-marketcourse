...
contract("CourseMarketplace", (accounts) => {
  ...
  describe("Transfer ownership", () => {
    ...
    it("should NOT transfer ownership when contract owner is not sending TX", async () => {
      await catchRevert(
        _contract.transferOwnership(accounts[2], { from: accounts[3] })
      );
    });
  ...
  describe("Deactivate course", () => {
    ...
    it("should have status of deactivated and price 0", async () => {
      ...
      beforeTxOwnerBalance = await getBalance(currentOwner);
      ...
      assert.equal(course.price, expectedPrice, "Course is NOT deactivated");
  ...
  describe("Repurchase course", () => {
    ...
    it("should NOT Repurchase when the course doesn't exist", async () => {
      const notExistingHast =
        "0x5ceb3f8075c3dbb5d490c8d1e6c950302ed065e1a9031750ad2c6513069e3fc3";
      await catchRevert(
        _contract.repurchaseCourse(notExistingHast, { from: buyer })
      );
    });
    ...
    it("should be able repurchase with the original buyer", async () => {
      ...
      const afterTxContractBalance = await getBalance(_contract.address);
      const gas = await getGas(result);
      course = await _contract.getCourseByHash(courseHash2);
      const exptecedState = 0;
      assert.equal(
        course.state,
        exptecedState,
        "The course is not purchased state"
      );
      assert.equal(
        course.price,
        value,
      ...
      assert.equal(
        ...
        afterTxBuyerBalance,
        "Client balance is not equal"
      );
      assert.equal(
        toBN(beforeTxContractBalance).add(toBN(value)).toString(),
        afterTxContractBalance,
        "Contract balance is not equal"
      );
    });
  ...
  describe("Normal withdraw", () => {
    ...
    const overLimitFunds = "9999990000000000000000";
    let currentOwner = null;
    before(async () => {
      currentOwner = await _contract.getContractOwner();
    ...
    it("should fail when withdrawing with Not owner address", async () => {
      const funds = "1000000000000000000";
    ...
    it("should have +1ETH after withdraw", async () => {
  ...
  describe("Emergencyy withdraw", () => {
    ...
    it("should have contract balance of 0", async () => {
      const contractBalance = await getBalance(_contract.address);
      assert.equal(contractBalance, 0, "Contract balance is not 0");
    });
  });
